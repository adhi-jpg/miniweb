...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["message"])) {
    ...
    if (!empty($message)) {
        ...
        $msg = $conn->query($sql) ? "✅ Announcement posted!" : "❌ Failed to post announcement: " . $conn->error;
    }
}
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        body {
            ...
            min-height: 100vh; color: #2c3e50; position: relative; overflow-x: hidden;
        }
        ...
        .shape:nth-child(1) { width: 80px; height: 80px; top: 15%; right: 20%; animation-delay: 0s; }
        .shape:nth-child(2) { width: 120px; height: 120px; top: 70%; left: 10%; animation-delay: -7s; }
        .shape:nth-child(3) { width: 60px; height: 60px; bottom: 30%; right: 30%; animation-delay: -14s; }
        ...
        .header {
            ...
            border-bottom: 1px solid rgba(255, 255, 255, 0.2); padding: 25px 50px;
            display: flex; justify-content: space-between; align-items: center;
            position: relative; z-index: 10; box-shadow: var(--shadow-light);
        ...
        .container {
            padding: 50px; max-width: 1200px; margin: 0 auto;
        ...
        .section-title {
            color: white; font-size: 32px; font-weight: 800; margin-bottom: 30px;
            text-shadow: 0 2px 10px rgba(0, 0, 0, 0.3);
            display: flex; align-items: center; gap: 15px;
        }

        .section-title i { font-size: 28px; color: #f39c12; }

        .announcement-form {
            background: rgba(255, 255, 255, 0.15); backdrop-filter: blur(20px);
            border: 1px solid rgba(255, 255, 255, 0.2); padding: 40px;
            border-radius: 25px; box-shadow: var(--shadow-medium);
        ...
        .form-group label {
            display: block; color: white; font-weight: 600; margin-bottom: 10px;
            font-size: 16px; text-transform: uppercase; letter-spacing: 0.5px;
        }

        .announcement-textarea {
            width: 100%; height: 150px; padding: 20px 25px;
            background: rgba(255, 255, 255, 0.1);
            border: 2px solid rgba(255, 255, 255, 0.2);
            border-radius: 15px; color: white; font-size: 16px; font-weight: 500;
            font-family: inherit; resize: vertical; transition: all 0.3s ease;
            backdrop-filter: blur(10px);
        }

        .announcement-textarea::placeholder { color: rgba(255, 255, 255, 0.6); }
        ...
        .submit-btn {
            padding: 18px 35px; background: var(--warning-gradient); color: white;
        ...
        .submit-btn:hover {
            transform: translateY(-3px); box-shadow: var(--shadow-heavy);
        }

        .submit-btn:active { transform: translateY(-1px); }

        .status-message {
            margin-top: 20px; padding: 18px 25px; border-radius: 12px;
        ...
        .status-message i { font-size: 18px; }

        .announcements-grid { display: grid; gap: 25px; }

        .announcement-card {
            background: rgba(255, 255, 255, 0.15); backdrop-filter: blur(20px);
            border: 1px solid rgba(255, 255, 255, 0.2); padding: 30px;
            border-radius: 20px; box-shadow: var(--shadow-light);
        ...
        .announcement-card::before {
            content: ''; position: absolute; top: 0; left: 0; right: 0; height: 4px;
            background: var(--primary-gradient); transform: scaleX(0);
            transition: transform 0.3s ease;
        }

        .announcement-card:hover {
            transform: translateY(-5px); box-shadow: var(--shadow-heavy);
        }
        ...
        .announcement-meta i { color: #f39c12; }

        .delete-btn {
            display: flex; align-items: center; gap: 8px; padding: 8px 15px;
            background: var(--danger-gradient); color: white; text-decoration: none;
        ...
        .announcement-date i { color: #f39c12; }
        ...
        .empty-state i {
            font-size: 48px; color: rgba(255, 255, 255, 0.5); margin-bottom: 20px;
        }

        .empty-state h3 {
            color: white; font-size: 24px; font-weight: 700; margin-bottom: 10px;
        }
        ...
        @media (max-width: 768px) {
            .header { padding: 20px 25px; flex-direction: column; gap: 15px; text-align: center; }
            .header h1 { font-size: 24px; }
            .header-actions { flex-direction: column; gap: 10px; }
            .container { padding: 30px 20px; }
            .section-title { font-size: 28px; flex-direction: column; text-align: center; gap: 10px; }
            .announcement-form { padding: 30px 25px; }
            .announcement-textarea { height: 120px; }
            .announcement-header { flex-direction: column; gap: 15px; }
            .announcement-meta { justify-content: center; }
        }
    </style>
</head>
<body>
